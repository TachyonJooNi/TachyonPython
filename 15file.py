# 예전 언어들에 비해 최신언어들이 파일을 읽어와서 사용하는데 편리하다.
# 모드 2가지 : 우리가 읽을수있는 텍스트모드(t)와 이미지나 mp3같은 별도의 프로그램이 필요한 바이너리(b)파일
# 계속 close() 주기 귀찮으면 with open() as 변수명 : 을 사용
# (Java) 직렬화, 역직렬화 => (Python) 피클링, 언피클링
