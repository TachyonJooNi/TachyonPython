...
# 파이썬에서는 문자열을 즉시 인덱싱 할 수 있으므로 길이 만큼 반복할 수 있다.
str1 = "파이썬이좋아요"
for ch in str1:
...
for i in range(3):
    print(i, end=" ")
    # 1번 실행후 break를 만나게 되므로 for문을 탈출한다.
    break
else:
    # 따라서 이경우 else구문이 실행되지 않는다.
...
-4로 나누어 떨어지는 해는 윤년, 그 빡의 해는 평년으로 한다.
-4로 나누어 떨어지지만 100으로도 나누어 떨어지는 해는 평년으로 한다.
-단, 400으로 나누어 떨어지는 해는 윤년으로 한다.(에: 2000년, 2400년)
"""
# 년, 월, 일을 숫자형으로 입력받는다.
